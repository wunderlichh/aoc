...
//let initialMemory = [| 0; 2; 7; 0 |]
let initialMemory = [|10; 3; 15; 10; 5; 15; 5; 15; 9; 2; 5; 8; 5; 2; 3; 6|]
        
let division (blocks: int) (banks: int) : int array =
    if blocks < (banks - 1)
    then
        Array.init banks (fun i -> if i < blocks then 1 else 0)
    else 
        let theDevision = Array.create banks (blocks / (banks - 1))
        theDevision.[theDevision.Length - 1] <- (blocks % (banks - 1))
        theDevision

let rec reallocate (cycle: int) (memory: int array) (states: int array list) : int =
    let tmpMem = Array.create memory.Length 0    
    let mostBlocks = Seq.max memory
    let mostBlocksIdx = Seq.findIndex ((=) mostBlocks) memory

    let toadd = division mostBlocks memory.Length
        
    [(mostBlocksIdx + 1)..(mostBlocksIdx + memory.Length)] |>
        Seq.iteri (fun di mi -> 
            let si = if mi < memory.Length then mi else abs (memory.Length - mi)
            tmpMem.[si] <- if si = mostBlocksIdx then toadd.[di] else memory.[si] + toadd.[di]) 

    if List.exists ((=) tmpMem) states 
    then cycle
    else reallocate (cycle + 1) tmpMem (memory::states)

printfn "Infinite loop after %d cycles detected! Reallocation terminated." (reallocate 1 initialMemory [])