...
let stringToSeqOfLines (str: string) =
    str.Trim().Split('\r', '\n') |> Seq.ofArray

let evaluateExp (state: Map<string, int>) (exp: string) : bool =
    let reg = exp.Substring(0, exp.IndexOf(" ")).Trim()
    let operation = exp.Substring(exp.IndexOf(" "), exp.LastIndexOf(" ") - exp.IndexOf(" ")).Trim()
    let value = exp.Substring(exp.LastIndexOf(" "), exp.Length - exp.LastIndexOf(" ")).Trim() |> int
    
    let rv = match state.TryFind reg with
             | Some v -> v
             | None -> 0
    ...
    | "<=" -> rv <= value
    | ">=" -> rv >= value
    | _ -> failwith (sprintf "Unkown operation: '%s'" operation)

let changeState (s: Map<string, int> * int) register value evaluate : Map<string, int> * int =
    let (state, max) = s
    if evaluate
    then
        match state.TryFind register with
        | Some v -> (Map.add register (v + value) state, if (v + value) > max then (v + value) else max)
        | None ->  (Map.add register value state, if value > max then value else max)
    else 
        state, max

let step (state: Map<string, int> * int) (ins: string) : Map<string, int> * int =
    let reg = ins.Substring(0, ins.IndexOf(' '))
    let cmd = ins.Substring(ins.IndexOf(' ') + 1, 3)
    let ifIdx = ins.IndexOf(" if ")
    let value = ins.Substring(ins.IndexOf(' ') + 4, ifIdx - (ins.IndexOf(' ') + 4)) |> int
    let expression = ins.Substring(ifIdx + 4, ins.Length - (ifIdx + 4))
    let ev = evaluateExp  (fst state) expression
    
    match cmd.ToLower() with 
    | "inc" -> changeState state reg value ev
    | "dec" -> changeState state reg (value * -1) ev
    | _ -> failwith (sprintf "Unknown command '%s'" cmd)

let answer =
    instructions
    |> stringToSeqOfLines
    |> Seq.fold step (Map.empty<string, int>, 0)
