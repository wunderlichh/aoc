...
let evaluateExp (state: Map<string, int>) (exp: string) : bool =
    ...
    let registerValue = match state.TryFind reg with
                        | Some v -> v
                        | None -> 0
...
let changeState (state: Map<string, int>) register value evaluate =
    if evaluate
    then
        match state.TryFind register with
        | Some v -> Map.add register (v + value) state
        | None ->  Map.add register value state
    else 
        state

let step (state: Map<string, int>) (ins: string) : Map<string, int> =
    ...
    let ev = evaluateExp state expression
...
let answer =
    instructions
    |> stringToSeqOfLines
    |> Seq.fold step Map.empty<string, int>
    |> Map.toSeq |> Seq.fold (fun (max) (k, v) -> if v > snd max then (k, v) else max) ("", 0)
