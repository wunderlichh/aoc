...
gotten themselves into a bit of trouble. A recursive algorithm has gotten out of hand, 
...
You ask each program to yell out their name, their weight, and (if they're holding a disc) the names 
of the programs immediately above them balancing on that disc. You write this information down (your puzzle input).
...
padx, and fwft. Those programs are, in turn, holding up other programs; in this example, none of those programs 
are holding up any other programs, and are all the tops of their own towers. 
...
expending all of their energy trying to keep the tower balanced. Apparently, one program has the wrong weight,
...
nodes above ugml are balanced, ugml itself is too heavy: it needs to be 8 units lighter for its stack to 
weigh 243 and keep the towers balanced. If this change were made, its weight would be 60.
...
let rec createStructure (element: string) (elementMap: Map<string, string>) : Tree =
    match elementMap.TryFind element with
    | Some ele -> 
        let idx = ele.IndexOf("(") + 1
        let idx' = ele.IndexOf(")")
        let ownWeight = ele.Substring(idx, idx' - idx) |> int

        if ele.IndexOf("->") = -1
        then Leaf (element, ownWeight)
        else
            let idx = ele.IndexOf("->") + 3
            let len = ele.Length - idx
            
            let subTrees = 
                ele.Substring(idx, len).Split(',')
                |> Seq.map (fun s -> s.Trim())
                |> Seq.map (fun s -> createStructure s elementMap)
                |> List.ofSeq

            Node (element, ownWeight, subTrees)

    | None -> failwith (sprintf "Element '%s' not found in map" element)
    
let createStructuredTree (input: string) : Tree =
    input
    |> (fun s -> s.Trim().Split('\r', '\n'))
    |> Seq.map (fun (s: string) -> (s.Substring(0, s.IndexOf(' ')), s))
    |> Map.ofSeq
    |> createStructure "vvsvez"

let rec sumTree (t: Tree) : int =
    match t with
    | Leaf (_, weight) -> weight
    | Node (_, weight, children) -> weight + (Seq.sumBy (fun c -> sumTree c) children)

let rec sumTreeWithoutOwnWeight (t: Tree) : int =
    match t with
    | Leaf (_, weight) -> weight
    | Node (_, weight, children) -> (Seq.sumBy (fun c -> sumTree c) children)

let printW (s: string) (t: Tree) : Unit = 
    match t with
    | Leaf (name, weight) -> printfn "%s Leaf (Name: %s, Own weight: %u)" s name weight
    | Node (name, weight, children) -> 
        let csum = Seq.sumBy (fun c -> sumTree c) children
        printfn "%s Node (Name: %s\t Own weight: %u\t Childs weight: %u\t Sum: %u)" s name weight csum (weight + csum) 

let rec searchUnbalancedNode (t: Tree) : Unit =
    match t with
    | Leaf (_, _) -> printW "LEAF RESULT -- " t
    | Node (_, _, children) -> 
        let childSums = Seq.map (sumTreeWithoutOwnWeight) children

        if Seq.forall ((=) (Seq.head childSums)) childSums 
        then printW "ALL_SAME RESULT -- " t
        else 
            
            let wrongItemsWeight = 
                childSums |> Seq.distinct |> List.ofSeq

            if wrongItemsWeight.Length > 1 then failwith "There must be only one distinct item"

            printfn "Distinct weight = %d" wrongItemsWeight.[0]

            Seq.iter (fun c -> if (sumTreeWithoutOwnWeight c) = wrongItemsWeight.[0] then searchUnbalancedNode c ) children
            

let tree = createStructuredTree input
searchUnbalancedNode tree