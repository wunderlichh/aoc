...
let testJumps = [| 0; 3; 0; 1; -3; |]

let rec step (steps: int) (position: int) (jumps: int array) : int =
    if position > jumps.Length - 1
    then 
        steps
    else 
        let offset = jumps.[position]
        let nextPosition = position + offset
        jumps.[position] <- jumps.[position] + (if offset >= 3 then -1 else 1)
        step (steps + 1) (nextPosition) jumps

printfn "Steps: %d" (step 0 0 jumps)
