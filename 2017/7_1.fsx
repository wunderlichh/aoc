...
let extractSubItems (row: string) =
    if row.IndexOf("->") > 0 
    then
        let idx = row.IndexOf("->") + 3
        let len = row.Length - idx
        Some (row.Substring(idx, len))
    else None

let subItems =
    input
    |> (fun s -> s.Trim().Split('\r', '\n'))
    |> Seq.choose extractSubItems
...
let answer = 
    input
    |> (fun s -> s.Trim().Split('\r', '\n'))
    |> Seq.map (fun (s: string) -> s.Substring(0, s.IndexOf(' ')))
    |> Seq.filter (fun i -> not (Seq.exists (fun j -> i.Equals(j)) subItems))
